...
Map_6::Map_6(sf::View* view):m_end(false), m_start(false), m_finished(false), m_learn(true), m_success(false) {
...
void Map_6::Inicialice(EventListener* listener) {
	m_listener = listener;
...
void Map_6::Render(sf::RenderWindow *window){
	window->draw(m_text);
	if (!Start()) {
		return;
	}
...
void Map_6::StartFinish() {
	m_clockEnd.restart();
	m_finished = true;
}
GameObject* Map_6::CreateGameObject(PhysicBody* pB, VisualBody* vB, Vec2 pos, Vec2 size) {
...
void Map_6::CreateEnemy(Vec2 pos) {
...
void Map_6::CreateMagnet(Vec2 pos) {
...
void Map_6::CreatePlayer(Vec2 pos) {
...
void Map_6::CreateRock(Vec2 pos, Vec2 dir) {
...
void Map_6::CreateMechanism(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_6::CreateDoor(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_6::CreateGoal(Vec2 pos, Vec2 size, int rotation) {
...
void Map_6::CreateWater(Vec2 pos, Vec2 size, int rotation) {
...
void Map_6::CreateWall(Vec2 pos, Vec2 size, int rotation) {
	PBWall *pwall = new PBWall;
	VWall  *vwall = new VWall;
	m_physiworld->CreateBody(pwall, pos, size);
	vwall->Initialize(size);
...
void Map_6::CreateDeadWall(Vec2 pos, Vec2 size, int rotation) {
...
void Map_6::CreateSoundWave(Vec2 pos, Vec2 dir, Vec2 size, int lifetime, int r, int g, int b, PhysicBody *pB) {
	if (!pB) {
...
Player* Map_6::GetPlayer() {
	return m_player.Get();
}

PVector<Mechanism>* Map_6::GetMechanisms() {
	return &m_mechanisms;
}
PVector<SoundWave>* Map_6::GetSoundWaves() {
	return &m_soundWaves;
}
PVector<Enemy>* Map_6::GetEnemies() {
	return &m_enemies;
}
PVector<Door>* Map_6::GetDoors() {
	return &m_doors;
}
PVector<Rock>* Map_6::GetRocks() {
	return &m_rocks;
}
PVector<Magnet>* Map_6::GetMagnets() {
	return &m_magnets;
}
bool Map_6::End() {
	return m_end;
...
void Map_6::UpdateEnemies() {
	for (auto it = m_enemies.GetBegin(); it != m_enemies.GetEnd(); it++) {
		(*it)->Update();
	}
}
void Map_6::UpdateRocks() {
	for (auto it = m_rocks.GetBegin(); it != m_rocks.GetEnd(); it++) {
		(*it)->Update();
	}
}
