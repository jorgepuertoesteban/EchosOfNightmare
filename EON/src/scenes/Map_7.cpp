...
Map_7::Map_7(sf::View* view):m_end(false), m_start(false), m_finished(false), m_learn(true), m_success(false) {
...
void Map_7::Inicialice(EventListener* listener) {
	m_listener = listener;
...
void Map_7::Render(sf::RenderWindow *window){
	window->draw(m_text);
	if (!Start()) {
		return;
	}
...
void Map_7::StartFinish() {
	m_clockEnd.restart();
	m_finished = true;
}
GameObject* Map_7::CreateGameObject(PhysicBody* pB, VisualBody* vB, Vec2 pos, Vec2 size) {
...
void Map_7::CreateEnemy(Vec2 pos) {
...
void Map_7::CreatePlayer(Vec2 pos) {
...
void Map_7::CreateMagnet(Vec2 pos) {
...
void Map_7::CreateRock(Vec2 pos, Vec2 dir) {
...
void Map_7::CreateDoor(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_7::CreateWater(Vec2 pos, Vec2 size, int rotation) {
...
void Map_7::CreateWall(Vec2 pos, Vec2 size, int rotation) {
	PBWall *pwall = new PBWall;
	VWall  *vwall = new VWall;
	m_physiworld->CreateBody(pwall, pos, size);
	vwall->Initialize(size);
...
void Map_7::CreateDeadWall(Vec2 pos, Vec2 size, int rotation) {
...
void Map_7::CreateSoundWave(Vec2 pos, Vec2 dir, Vec2 size, int lifetime, int r, int g, int b, PhysicBody *pB) {
	if (!pB) {
...
PVector<Enemy>* Map_7::GetEnemies() {
	return &m_enemies;
}
PVector<Door>* Map_7::GetDoors() {
	return &m_doors;
}
PVector<Rock>* Map_7::GetRocks() {
	return &m_rocks;
}
PVector<Magnet>* Map_7::GetMagnets() {
	return &m_magnets;
}
bool Map_7::End() {
	return m_end;
...
void Map_7::UpdateEnemies() {
	for (auto it = m_enemies.GetBegin(); it != m_enemies.GetEnd(); it++) {
		(*it)->Update();
	}
}
void Map_7::UpdateRocks() {
	for (auto it = m_rocks.GetBegin(); it != m_rocks.GetEnd(); it++) {
		(*it)->Update();
	}
}
