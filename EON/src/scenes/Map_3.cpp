...
void Map_3::Inicialice(EventListener* listener) {
	m_listener = listener;
...
void Map_3::Render(sf::RenderWindow *window){
	window->draw(m_text);
	if (!Start()) {
		return;
	}
...
void Map_3::StartFinish() {
	m_clockEnd.restart();
	m_finished = true;
}
GameObject* Map_3::CreateGameObject(PhysicBody* pB, VisualBody* vB, Vec2 pos, Vec2 size) {
...
void Map_3::CreateEnemy(Vec2 pos) {
...
void Map_3::CreatePlayer(Vec2 pos) {
...
void Map_3::CreateMagnet(Vec2 pos) {
...
void Map_3::CreateRock(Vec2 pos, Vec2 dir) {
...
void Map_3::CreateMechanism(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_3::CreateWater(Vec2 pos, Vec2 size, int rotation) {
...
void Map_3::CreateWall(Vec2 pos, Vec2 size, int rotation) {
	PBWall *pwall = new PBWall;
	VWall  *vwall = new VWall;
	m_physiworld->CreateBody(pwall, pos, size);
	vwall->Initialize(size);
...
void Map_3::CreateDeadWall(Vec2 pos, Vec2 size, int rotation) {
...
void Map_3::CreateSoundWave(Vec2 pos, Vec2 dir, Vec2 size, int lifetime, int r, int g, int b, PhysicBody *pB) {
	if (!pB) {
...
Player* Map_3::GetPlayer() {
	return m_player.Get();
}

PVector<SoundWave>* Map_3::GetSoundWaves() {
	return &m_soundWaves;
}
PVector<Enemy>* Map_3::GetEnemies() {
	return &m_enemies;
}
PVector<Rock>* Map_3::GetRocks() {
	return &m_rocks;
}
PVector<Mechanism>* Map_3::GetMechanisms() {
	return &m_mechanisms;
}
PVector<Magnet>* Map_3::GetMagnets() {
	return &m_magnets;
}
