...
Map_5::Map_5(sf::View* view):m_end(false), m_start(false), m_finished(false), m_learn(true), m_success(false) {
...
void Map_5::Inicialice(EventListener* listener) {
	m_listener = listener;
...
void Map_5::Render(sf::RenderWindow *window){
	window->draw(m_text);
	if (!Start()) {
		return;
	}
...
void Map_5::StartFinish() {
	m_clockEnd.restart();
	m_finished = true;
}
GameObject* Map_5::CreateGameObject(PhysicBody* pB, VisualBody* vB, Vec2 pos, Vec2 size) {
...
void Map_5::CreateEnemy(Vec2 pos) {
...
void Map_5::CreatePlayer(Vec2 pos) {
...
void Map_5::CreateMagnet(Vec2 pos) {
...
void Map_5::CreateRock(Vec2 pos, Vec2 dir) {
...
void Map_5::CreateMechanism(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_5::CreateDoor(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_5::CreateGoal(Vec2 pos, Vec2 size, int rotation) {
...
void Map_5::CreateWater(Vec2 pos, Vec2 size, int rotation) {
...
void Map_5::CreateWall(Vec2 pos, Vec2 size, int rotation) {
	PBWall *pwall = new PBWall;
	VWall  *vwall = new VWall;
	m_physiworld->CreateBody(pwall, pos, size);
	vwall->Initialize(size);
...
void Map_5::CreateDeadWall(Vec2 pos, Vec2 size, int rotation) {
...
void Map_5::CreateSoundWave(Vec2 pos, Vec2 dir, Vec2 size, int lifetime, int r, int g, int b, PhysicBody *pB) {
	if (!pB) {
...
Player* Map_5::GetPlayer() {
	return m_player.Get();
}

PVector<Mechanism>* Map_5::GetMechanisms() {
	return &m_mechanisms;
}
PVector<SoundWave>* Map_5::GetSoundWaves() {
	return &m_soundWaves;
}
PVector<Door>* Map_5::GetDoors() {
	return &m_doors;
}
PVector<Enemy>* Map_5::GetEnemies() {
	return &m_enemies;
}
PVector<Rock>* Map_5::GetRocks() {
	return &m_rocks;
}
PVector<Magnet>* Map_5::GetMagnets() {
	return &m_magnets;
}
bool Map_5::End() {
	return m_end;
...
void Map_5::UpdateEnemies() {
	for (auto it = m_enemies.GetBegin(); it != m_enemies.GetEnd(); it++) {
		(*it)->Update();
	}
}
void Map_5::UpdateRocks() {
	for (auto it = m_rocks.GetBegin(); it != m_rocks.GetEnd(); it++) {
		(*it)->Update();
	}
}
void Map_5::UpdateDoors() {
