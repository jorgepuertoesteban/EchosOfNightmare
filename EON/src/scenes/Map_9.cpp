...
Map_9::Map_9(sf::View* view):m_end(false), m_start(false), m_finished(false), m_learn(true), m_success(false) {
...
void Map_9::Inicialice(EventListener* listener) {
	m_listener = listener;
...
void Map_9::Render(sf::RenderWindow *window){
	window->draw(m_text);
	if (!Start()) {
		return;
	}
...
void Map_9::StartFinish() {
	m_clockEnd.restart();
	m_finished = true;
}
GameObject* Map_9::CreateGameObject(PhysicBody* pB, VisualBody* vB, Vec2 pos, Vec2 size) {
...
void Map_9::CreateEnemy(Vec2 pos) {
...
void Map_9::CreatePlayer(Vec2 pos) {
...
void Map_9::CreateRock(Vec2 pos, Vec2 dir) {
...
void Map_9::CreateGoal(Vec2 pos, Vec2 size, int rotation) {
...
void Map_9::CreateMagnet(Vec2 pos) {
...
void Map_9::CreateMechanism(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_9::CreateDoor(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_9::CreateWater(Vec2 pos, Vec2 size, int rotation) {
...
void Map_9::CreateWall(Vec2 pos, Vec2 size, int rotation) {
	PBWall *pwall = new PBWall;
	VWall  *vwall = new VWall;
	m_physiworld->CreateBody(pwall, pos, size);
	vwall->Initialize(size);
...
void Map_9::CreateDeadWall(Vec2 pos, Vec2 size, int rotation) {
...
void Map_9::CreateSoundWave(Vec2 pos, Vec2 dir, Vec2 size, int lifetime, int r, int g, int b, PhysicBody *pB) {
	if (!pB) {
...
Player* Map_9::GetPlayer() {
	return m_player.Get();
}
PVector<SoundWave>* Map_9::GetSoundWaves() {
	return &m_soundWaves;
}
PVector<Mechanism>* Map_9::GetMechanisms() {
	return &m_mechanisms;
}
PVector<Enemy>* Map_9::GetEnemies() {
	return &m_enemies;
}
PVector<Rock>* Map_9::GetRocks() {
	return &m_rocks;
}
PVector<Magnet>* Map_9::GetMagnets() {
	return &m_magnets;
}
PVector<Door>* Map_9::GetDoors() {
	return &m_doors;
}
bool Map_9::End() {
	return m_end;
...
void Map_9::UpdateEnemies() {
	for (auto it = m_enemies.GetBegin(); it != m_enemies.GetEnd(); it++) {
		(*it)->Update();
	}
}
void Map_9::UpdateRocks() {
	for (auto it = m_rocks.GetBegin(); it != m_rocks.GetEnd(); it++) {
		(*it)->Update();
	}
}
void Map_9::UpdateMechanisms() {
