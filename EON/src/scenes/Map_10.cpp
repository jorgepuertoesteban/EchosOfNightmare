...
Map_10::Map_10(sf::View* view):m_end(false), m_start(false), m_finished(false), m_learn(true), m_success(false) {
...
void Map_10::Inicialice(EventListener* listener) {
	m_listener = listener;
...
void Map_10::Render(sf::RenderWindow *window){
	window->draw(m_text);
	if (!Start()) {
		return;
	}
...
void Map_10::StartFinish() {
	m_clockEnd.restart();
	m_finished = true;
}
GameObject* Map_10::CreateGameObject(PhysicBody* pB, VisualBody* vB, Vec2 pos, Vec2 size) {
...
void Map_10::CreateEnemy(Vec2 pos) {
...
void Map_10::CreatePlayer(Vec2 pos) {
...
void Map_10::CreateRock(Vec2 pos, Vec2 dir) {
...
void Map_10::CreateGoal(Vec2 pos, Vec2 size, int rotation) {
...
void Map_10::CreateMagnet(Vec2 pos) {
...
void Map_10::CreateMechanism(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_10::CreateDoor(Vec2 pos, Vec2 size, int rotation, int door) {
...
void Map_10::CreateWater(Vec2 pos, Vec2 size, int rotation) {
...
void Map_10::CreateWall(Vec2 pos, Vec2 size, int rotation) {
	PBWall *pwall = new PBWall;
	VWall  *vwall = new VWall;
	m_physiworld->CreateBody(pwall, pos, size);
	vwall->Initialize(size);
...
void Map_10::CreateDeadWall(Vec2 pos, Vec2 size, int rotation) {
...
void Map_10::CreateSoundWave(Vec2 pos, Vec2 dir, Vec2 size, int lifetime, int r, int g, int b, PhysicBody *pB) {
	if (!pB) {
...
Player* Map_10::GetPlayer() {
	return m_player.Get();
}
PVector<SoundWave>* Map_10::GetSoundWaves() {
	return &m_soundWaves;
}
PVector<Mechanism>* Map_10::GetMechanisms() {
	return &m_mechanisms;
}
PVector<Enemy>* Map_10::GetEnemies() {
	return &m_enemies;
}
PVector<Rock>* Map_10::GetRocks() {
	return &m_rocks;
}
PVector<Magnet>* Map_10::GetMagnets() {
	return &m_magnets;
}
PVector<Door>* Map_10::GetDoors() {
	return &m_doors;
}
bool Map_10::End() {
	return m_end;
...
void Map_10::UpdatePlayer() {

	m_player.Get()->Update();

	Vec2 itWc = m_player.Get()->GetPosition() / 64.f;
	float itM{ 0.1f };
	...
	while (itMg != m_magnets.GetEnd()) {
		...
		float force = 9.8f * mM * itM / r;

		delta.Normalize();
		Vec2 delta2{ force *delta.x, force *delta.y };
		m_player.Get()->ApplyForce(delta2);
...
void Map_10::UpdateEnemies() {
	for (auto it = m_enemies.GetBegin(); it != m_enemies.GetEnd(); it++) {
		(*it)->Update();
	}
}
void Map_10::UpdateRocks() {
	for (auto it = m_rocks.GetBegin(); it != m_rocks.GetEnd(); it++) {
		(*it)->Update();
	}
}
void Map_10::UpdateMechanisms() {
